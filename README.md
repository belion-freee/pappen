...
- ユーザー登録(既にメンバー登録済みの場合はこの作業は不要)
  ...
  - 手動で追加する場合、グループ内で`ぱっぷん ユーザー登録`とメッセージを送信して、参加するメンバーに`はい`ボタンを押してもらう。
- `ぱっぷん イベント 作成`とメッセージを送信
- ぱっぷんが作ったイベントの雛形を編集する。
...
- 次からは`ぱっぷん イベント`とメッセージを送信するとイベント機能が表示される。普通にリンクをコピってもいい。
- 役目を終えたら`ぱっぷんもう大丈夫だよ`とメッセージを送信すると、ぱっぷんは退出する。
...
### 退出
`ぱっぷんもう大丈夫だよ`とメッセージを送信すると、チャットルームからぱっぷんは退出する。
